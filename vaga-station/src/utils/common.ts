...
export const convertToTokenString = (amount: string, decimal: number) => {
  return VagaUtil.getTokenStringFromUToken(convertNumber(amount), convertNumber(decimal));
};

export const convertToTokenNumber = (amount: number | string, decimal: string) => {
  return convertNumber(VagaUtil.getTokenStringFromUToken(convertNumber(amount), convertNumber(decimal)));
};

export const convertToFctString = (uFctAmount: string) => {
  return VagaUtil.getFCTStringFromUFCTStr(uFctAmount);
};

export const convertToFctNumber = (uFctAmount: number | string) => {
  return convertNumber(VagaUtil.getFCTStringFromUFCTStr(uFctAmount.toString()));
};
