...
export class ContractQueryClient {
    ...
    async queryGetContractListFromHash(contractHash: string): Promise<string[]> {
        const path = "/vagachain/vagachain/contract/ContractListFromHash" + "/" + contractHash;
    ...
    async queryIsContractOwner(fileHash: string, ownerAddress: string): Promise<boolean> {
        const path = "/vagachain/vagachain/contract/isContractOwner";
    ...
    async queryContractFile(fileHash: string): Promise<ContractFileType> {
        const path = `/vagachain/vagachain/contract/contractFile/${fileHash}`;
        const result = await this.axios.get(path);
    ...
    async queryContractFileAll(paginationKey: string = ""): Promise<{
        ...
        const path = "/vagachain/vagachain/contract/contractFile";
    ...
    async queryContractLog(logId: string): Promise<ContractLogType> {
        const path = `/vagachain/vagachain/contract/contractLog/${logId}`;
        const result = await this.axios.get(path);
    ...
    async queryContractLogAll(paginationKey: string = ""): Promise<{
        ...
        pagination: Pagination; }> {
        const path = "/vagachain/vagachain/contract/contractLog";
        const result = await this.axios.get(path, { params: { "pagination.key": paginationKey } });
