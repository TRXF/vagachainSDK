...
import { ContractService } from '../sdk/VagaContractService';
...
describe('[21. Token Tx Test]', () => {
	...
	it('Token CreateToken', async () => {
		...
		var result = await vaga.Token.createToken(wallet, tokenName, symbol, tokenURI, totalSupply, decimal, mintable, burnable);
	...
	it('Token Mint', async () => {
		...
		var result = await vaga.Token.mint(wallet, tokenID, amount, decimal, toAddress);
	...
	it('Token Burn', async () => {
		...
		var result = await vaga.Token.burn(wallet, tokenID, amount, decimal);
	...
	it('Token UpdateTokenURI', async () => {
		...
		const tokenURI = "https://vagachain.com";

		var result = await vaga.Token.updateTokenURI(wallet, tokenID, tokenURI);
	...
	it('bank sendToken OK', async () => {
		...
		var result = await vaga.Bank.sendToken(wallet, bobAddress, tokenID, amount, decimal);
