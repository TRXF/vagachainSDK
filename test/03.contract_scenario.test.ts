...
describe('[03. Contract scenario base test]', () => {
	...
	it('CreateContract add', async () => {
		...
		const donueWallet = await vaga.Wallet.fromMnemonic(donueMnemonic);
		const creatorWallet = await vaga.Wallet.fromMnemonic(creatorMnemonic);
		...
		var result = await vaga.Contract.addContractLog(donueWallet, contractHash, timeStamp, eventName, ownerAddress, jsonString);
	...
	it('AddSigner - each tx add', async () => {
		...
		const donueWallet = await vaga.Wallet.fromMnemonic(donueMnemonic);
		const signer1Wallet = await vaga.Wallet.fromMnemonic(signer1Mnemonic);
		const signer2Wallet = await vaga.Wallet.fromMnemonic(signer2Mnemonic);
		const signer3Wallet = await vaga.Wallet.fromMnemonic(signer3Mnemonic);
		const signer4Wallet = await vaga.Wallet.fromMnemonic(signer4Mnemonic);
		...
		var result = await vaga.Contract.addContractLog(donueWallet, contractHash, timeStamp, eventName, ownerAddress1, jsonString);
		expect(result.code).equal(0);

		result = await vaga.Contract.addContractLog(donueWallet, contractHash, timeStamp, eventName, ownerAddress2, jsonString);
		expect(result.code).equal(0);

		result = await vaga.Contract.addContractLog(donueWallet, contractHash, timeStamp, eventName, ownerAddress3, jsonString);
		expect(result.code).equal(0);

		result = await vaga.Contract.addContractLog(donueWallet, contractHash, timeStamp, eventName, ownerAddress4, jsonString);
		expect(result.code).equal(0);
	...
	it('AddSigner - all tx to one sign', async () => {
		...
		var msg1 = await vaga.Contract.getUnsignedTxAddContractLog(donueWallet, contractHash, timeStamp, eventName, ownerAddress1, jsonString);
		var msg2 = await vaga.Contract.getUnsignedTxAddContractLog(donueWallet, contractHash, timeStamp, eventName, ownerAddress2, jsonString);
		var msg3 = await vaga.Contract.getUnsignedTxAddContractLog(donueWallet, contractHash, timeStamp, eventName, ownerAddress3, jsonString);
		var msg4 = await vaga.Contract.getUnsignedTxAddContractLog(donueWallet, contractHash, timeStamp, eventName, ownerAddress4, jsonString);

		let result = await vaga.Contract.signAndBroadcast(donueWallet, [msg1, msg2, msg3, msg4]);
		expect(result.code).equal(0);
	});
	...
	it('DestroyContract - after reject, destroy contract', async () => {
		...
		var result = await vaga.Contract.addContractLog(donueWallet, contractHash, timeStamp, eventName, creatorAddress, jsonString);
		expect(result.code).equal(0);
	...
	it('createContractFile - CompleteContract, after private sign, write to chain by new message', async () => {
		...
		let timeStamp = Math.round(+new Date() / 1000);;
		let fileHash = await VagaUtil.getFileHash("./test/sample/sample_contract.pdf") + timeStamp;

		//console.log("fileHash : " + fileHash);

		let ipfsHash = await vaga.Ipfs.addJson(fileHash);
		...
		var result = await vaga.Contract.createContractFile(donueWallet, fileHash, timeStamp, [ownerAddress1, ownerAddress2, ownerAddress3, ownerAddress4], jsonString);
		expect(result.code).equal(0);

		var contractFile = await vaga.Contract.getContractFile(fileHash);
		...
		//console.log("contract file url:" + vaga.Ipfs.getURLFromHash(decryptHash1));
	});
