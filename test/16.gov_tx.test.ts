...
describe.skip('[16. Gov Tx Test]', () => {
	...
	/*it.skip('SubmitSoftwareUpgradeProposalByTime Test', async () => {
		...
		var result = await vaga.Gov.SubmitSoftwareUpgradeProposalByTime(aliceWallet, title, description, initialDepositFCT, upgradeName, upgradeTime);
	...
	it('Vote - bob NO', async () => {
		...
		const wallet = await vaga.Wallet.fromMnemonic(bobMnemonic);
		const proposalId = tempProposalId;

		var result = await vaga.Gov.vote(wallet, proposalId, VotingOption.VOTE_OPTION_NO);
		//console.log(result);
		expect(result.code).to.equal(0);
	});
