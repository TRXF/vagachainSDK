...
describe('[12. Staking Tx Test]', () => {
	...
	it('redelegate OK', async () => {
		...
		let result = await vaga.Staking.delegate(wallet, srcValidatorAddress, amount);
		expect(result.code).to.equal(0);

		// NOTICE: there's a case for use more than 200000 gas here.
		let result1 = await vaga.Staking.redelegate(wallet, srcValidatorAddress, dstValidatorAddress, amount,
	...
	it('staking send Fail - send zero money', async () => {
		...
		try {
			var result = await vaga.Staking.delegate(wallet, validatorAddress, amount);
		}
		catch (error) {
	...
	// NOTICE: not decide to include vaga.js spec
	it.skip('editValidator OK', async () => {
		...
		var result = await vaga.Staking.editValidator(wallet, validatorAddress, description, commissionRate, minSelfDelegation);
